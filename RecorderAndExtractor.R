...
record <- function(seconds = 10)  {
  if(f %in% list.files()) 
{file.remove(f); print('The former file has been replaced');}
require(audio)
s11 <- rep(NA_real_, 16000*kk) # rate=16000
record(s11, 16000, seconds)  # record in mono mode
wait(kk)
save.wave(s11,f)
}
}

### Sets up directory to be ready for processing ####
...
###Function that extracts information from all wav files in inputted folder###
...
data <- processFolder('C:/Users/Jabari/Downloads/')
